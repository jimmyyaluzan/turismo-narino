...
function mostrarHistorial(lugares) {
    let contenido = "";
    if (lugares.length >= 1) {
        $.each(lugares, function (index, lugar) {
            ...
            contenido += '<tr><th scope="row">' + lugar.id + '</th>' +
                    '<td>' + lugar.sitio + '</td>' +
                    '<td>' + lugar.clima+ '</td>' +
                    '<td><input type="checkbox" name="novedad" id="novedad' + lugar.id 
                    + '" disabled ';
            if (lugar.novedad) {
                contenido += 'checked'
            }
            contenido += '></td><td>' + lugar.fechaVenta + '</td>' +
                    '<td><button id="devolver-btn" onclick= "devolverLugar(' + lugar.id 
                    + ');" class="btn btn-danger">Devolver lugar</button></td></tr>';
...
function devolverLugar(id) {

    $.ajax({
        type: "GET",
        dataType: "html",
        url: "./ServletLugarDevolver",
        data: $.param({
            username: username,
            id: id,
        }),
        success: function (result) {
            ...
            if (result != false) {
                ...
                console.log("Error devolviendo el lugar");
            }
        }
    });
